...
// Per-layer "post-QKV" fused kernel: Q rms_norm + scale + RoPE writes the
// post-rope Q back to its F32 buffer; K rms_norm + scale + RoPE writes
// post-rope K (F16) directly into k_cache[pos]; V copies (F32→F16)
// directly into v_cache[pos]. Replaces 8 launches per code-pred layer
// (Q-rms_norm, Q-mul, K-rms_norm, K-mul, Q-rope, K-rope, K-set_rows,
// V-set_rows) with 1.
//
// Grid:  (max(N_HEADS, N_KV_HEADS), 3, 1)  — blockIdx.y selects {Q,K,V}.
// Block: (HEAD_DIM threads). Block-internal sync only (no grid sync).
//
// HEAD_DIM must be ≤ 1024 and a multiple of 32. NeoX-style RoPE
// (half-rotation pairing).
template <int HEAD_DIM, int N_HEADS, int N_KV_HEADS>
void launch_fused_qknorm_rope_setrows_f16(
    float *       qcur_io,           // [HEAD_DIM, N_HEADS]  (F32, in-place)
    const float * kcur_in,           // [HEAD_DIM, N_KV_HEADS]
    const float * vcur_in,           // [HEAD_DIM, N_KV_HEADS]
    const float * q_norm_w,          // [HEAD_DIM]
    const float * k_norm_w,          // [HEAD_DIM]
    void *        k_cache_f16,       // [HEAD_DIM*N_KV_HEADS, n_ctx] F16
    void *        v_cache_f16,
    const int *   pos_ptr,           // device ptr to int32 [1]
    float         eps,
    float         theta_base,
    cudaStream_t  stream);
