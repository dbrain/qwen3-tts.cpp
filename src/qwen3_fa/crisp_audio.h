...
// VRAM-probe helper. Reports bytes held by the audio tower's GPU buffers
// (model weights, conv-front-end gallocr, body sched compute buf). Zeros
// if the backend is CPU. body_sched_gpu_bytes reflects the high-water
// mark across encode calls.
struct crisp_audio_vram_stats {
    size_t model_buf_bytes;
    size_t conv_galloc_bytes;
    size_t body_sched_gpu_bytes;
};
void crisp_audio_get_vram_stats(struct crisp_audio_context* ctx, struct crisp_audio_vram_stats* out);

// Opaque GPU backend handle. NULL if the audio tower is CPU-only. Used by
// callers that need to introspect backend-specific state (e.g. cudaGraph
// capture cache) without pulling ggml-backend.h into their headers.
