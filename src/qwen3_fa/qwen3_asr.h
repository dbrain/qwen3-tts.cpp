...
// context, including the nested crisp_audio tower. Used by HANDOFF-fa-aligner-vram
// Phase A to reconcile per-pid VRAM against known allocations and isolate
// the "other" delta (CUDA primary context, cublas workspace, cudaGraph cache).
//
// All sizes are bytes. CPU-spill weights are reported separately for clarity
// even though they don't sit on the GPU.

struct qwen3_asr_vram_breakdown {
    size_t model_buf_bytes;                 // LLM-body weights on GPU
    size_t model_buf_cpu_bytes;             // LLM-body weights spilled to CPU (CRISPASR_N_GPU_LAYERS)
    size_t fused_buf_bytes;                 // fused QKV weights
    size_t kv_buf_bytes;                    // KV cache
    size_t sched_buf_gpu_bytes;             // shared ggml_backend_sched compute buf (HWM)
    size_t audio_model_buf_bytes;           // crisp_audio tower weights on GPU
    size_t audio_conv_galloc_bytes;         // conv-frontend dedicated gallocr
    size_t audio_body_sched_gpu_bytes;      // audio-body sched compute buf (HWM)
};

void qwen3_asr_get_vram_breakdown(struct qwen3_asr_context* ctx, struct qwen3_asr_vram_breakdown* out);

// Opaque GPU backend handle. NULL if running on CPU. Used by callers that
// want backend-specific introspection (e.g. cudaGraph capture cache counts)
// without including ggml-backend.h.
void* qwen3_asr_get_gpu_backend_handle(struct qwen3_asr_context* ctx);
